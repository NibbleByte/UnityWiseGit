...
namespace DevLocker.VersionControl.WiseGit.LockPrompting
{
	...
	/// </summary>
	public class GitLockPromptWindow : EditorWindow
	{
		[Serializable]
		private class LockEntryData
		{
			public GitStatusData StatusData;
...
#pragma warning restore CA2235
			...
			public LockEntryData(GitStatusData statusData)
			{
		...
		public static void PromptLock(IEnumerable<GitStatusData> shouldLockEntries, IEnumerable<GitStatusData> lockedByOtherEntries)
		{
			if (GitPreferencesManager.Instance.TemporarySilenceLockPrompts)
				return;
			...
			if (GitPreferencesManager.Instance.PersonalPrefs.AutoLockOnModified) {
				...
				shouldLockEntries = new List<GitStatusData>();
		...
		private void AppendEntriesToLock(IEnumerable<GitStatusData> entries)
		{
			var toAdd = entries
		...
		private void InitializeStyles()
		{
			...
			GitPreferencesWindow.MigrateButtonStyleToUIElementsIfNeeded(MiniIconButtonlessStyle);
		}
		...
		void OnGUI()
		{
			...
			var silenceContent = new GUIContent("Silence!", $"Suppress all lock prompts and auto-lock actions until Unity is restarted or by selecting \"{GitOverlayIcons.InvalidateDatabaseMenuText.Replace("&&", "&")}\"");
			if (GUILayout.Button(silenceContent, EditorStyles.toolbarButton)) {
				if (EditorUtility.DisplayDialog("Silence Lock Prompts?", $"{silenceContent.tooltip}\n\nUseful if you want to test stuff locally without committing later.\n\nAre you sure?", "Yes", "No")) {
					GitPreferencesManager.Instance.TemporarySilenceLockPrompts = true;
			...
			EditorGUILayout.HelpBox(
				...
				$"To force re-evaluate all of the locks, select the \"{GitOverlayIcons.InvalidateDatabaseMenuText.Replace("&&", "&")}\" menu.",
			...
			foreach (var lockEntry in m_LockEntries) {
				...
				GitStatusData statusData = lockEntry.StatusData;
				...
				if (GUILayout.Button(m_DiffContent, MiniIconButtonlessStyle, GUILayout.Width(RevertSize), GUILayout.Height(RevertSize))) {
					if (!string.IsNullOrEmpty(statusData.MovedTo)) {
						GitContextMenusManager.DiffAsset(statusData.MovedTo);
					} else {
						GitContextMenusManager.DiffAsset(statusData.Path);
					}
				}
			...
			if (GUILayout.Button(lockSelectedButtonText)) {
				...
				if (selectedStatusData.Any()) {
					GitLockPromptDatabase.Instance.LockEntries(selectedStatusData, m_AllowStealingLocks);
				}
				Close();
			}
			GUI.backgroundColor = prevBackgroundColor;
